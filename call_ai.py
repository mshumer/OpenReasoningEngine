    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_candidates) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_candidate):
            try:
                ...
                print(f"{Fore.GREEN}Successfully generated candidate {len(candidates)}/{num_candidates}{Style.RESET_ALL}")
            except Exception as e:
                print(f"{Fore.RED}Error generating candidate: {str(e)}{Style.RESET_ALL}")

    print(f"{Fore.GREEN}Generated {len(candidates)} candidates successfully{Style.RESET_ALL}")
    return candidates
...
def generate_best_candidate(
    ...
    candidates = generate_multiple_candidates(
        task, messages, api_key, tools, num_candidates, model, temperature, top_p, max_tokens, api_url, verbose, is_first_step
    )
...
Think it through inside the <thinking> section, and then output the number of the candidate that is most likely to lead to solving the <task> properly in the <best_candidate_number> section. In the <best_candidate_number> section, only output the number, nothing else. Possible numbers are: {', '.join(str(i) for i in range(1, num_candidates + 1))}"""
    ...
    best_candidate_response = send_message_to_api(
        task="",
        messages=[{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": evaluation_prompt}],
        api_key=api_key,
        tools=tools,
    )

    # Parse the best candidate number from the response
    best_candidate_number = int(best_candidate_response['content'].split('<best_candidate_number>')[1].split('</best_candidate_number>')[0].strip())
